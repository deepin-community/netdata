...
get_config_value() {
    ...
    if [ "${NETDATA_RUNNING}" -eq 1 ]; then
        config_result=$(/usr/sbin/netdatacli 2>/dev/null read-config "$conf_file|$section|$key_name"; exit $?)
        result="$?"
        if [ "${result}" -ne 0 ]; then
    ...
    if [ "${NETDATA_RUNNING}" -eq 0 ]; then
        config_result=$(/usr/sbin/netdata 2>/dev/null -W get2 "$conf_file" "$section" "$key_name" unknown_default)
    fi
    echo "$config_result"
}
...
[ -z "${NETDATA_VARLIB_DIR}" ] && NETDATA_VARLIB_DIR="/var/lib/netdata"
MACHINE_GUID_FILE="/var/lib/netdata/registry/netdata.public.unique.id"
...
HERE_DOC
        ...
        /usr/sbin/netdatacli write-config 'cloud|global|enabled|yes' && \
        /usr/sbin/netdatacli write-config "cloud|global|cloud base url|$URL_BASE" && \
        /usr/sbin/netdatacli reload-claiming-state && \
        if [ "${HTTP_STATUS_CODE}" = "204" ] ; then
